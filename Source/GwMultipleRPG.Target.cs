...
public class GwMultipleRPGTarget : TargetRules
{
	public GwMultipleRPGTarget(TargetInfo Target) : base(Target)
	{
		Type = TargetType.Game;
		ExtraModuleNames.Add("GwMultipleRPG");

		GwMultipleRPGTarget.ApplySharedGwTargetSettings(this);
	}

	private static bool bHasWarnedAboutShared = false;

	internal static void ApplySharedGwTargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			GwMultipleRPGTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("GwGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}
