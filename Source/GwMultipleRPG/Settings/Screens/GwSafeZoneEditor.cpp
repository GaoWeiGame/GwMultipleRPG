...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GwSafeZoneEditor)
...
UGwSafeZoneEditor::UGwSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UGwSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UGwSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UGwSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &UGwSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UGwSafeZoneEditor::HandleBackClicked);
	}
}

bool UGwSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UGwSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UGwSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UGwSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UGwSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UGwSettingsLocal::Get()->GetSafeZone());
}

void UGwSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		UGwSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
