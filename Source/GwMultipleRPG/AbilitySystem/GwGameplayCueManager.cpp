...
#include "GwGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GwGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EGwEditorLoadMode
{
...
namespace GwGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Gw.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via GwGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UGwGameplayCueManager::DumpGameplayCues));

	static EGwEditorLoadMode LoadMode = EGwEditorLoadMode::LoadUpfront;
}
...
UGwGameplayCueManager::UGwGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGwGameplayCueManager* UGwGameplayCueManager::Get()
{
	return Cast<UGwGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGwGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UGwGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogGw, Warning, TEXT("UGwGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGwGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (GwGameplayCueManagerCvars::LoadMode)
	{
	case EGwEditorLoadMode::LoadUpfront:
		return true;
	case EGwEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EGwEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UGwGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGwGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGwGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UGwGameplayCueManager* GCM = Cast<UGwGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogGw, Error, TEXT("DumpGameplayCues failed. No UGwGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogGw, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogGw, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogGw, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogGw, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogGw, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogGw, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogGw, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogGw, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogGw, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogGw, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogGw, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogGw, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UGwGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGwGameplayCueManager* StrongThis = Get())
					{
...
void UGwGameplayCueManager::HandlePostGarbageCollect()
{
...
void UGwGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogGw, Warning, TEXT("UGwGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGwGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (GwGameplayCueManagerCvars::LoadMode)
	{
	case EGwEditorLoadMode::LoadUpfront:
...
void UGwGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGwGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGwGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGwGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UGwGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UGwGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
