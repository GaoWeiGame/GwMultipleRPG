...
#include "GwMultipleRPG/Equipment/GwPickupDefinition.h"
#include "GwMultipleRPG/Inventory/GwInventoryItemDefinition.h"
#include "GwMultipleRPG/Inventory/InventoryFragment_SetStats.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GwWeaponSpawner)
...
// Sets default values
AGwWeaponSpawner::AGwWeaponSpawner()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &AGwWeaponSpawner::OnOverlapBegin);
...
void AGwWeaponSpawner::BeginPlay()
{
	Super::BeginPlay();
	...
	else if (const UWorld* World = GetWorld())
	{
		...
		{
			UE_LOG(LogGw, Error, TEXT("'%s' does not have a valid weapon definition! Make sure to set this data on the instance!"), *GetNameSafe(this));	
		}
	}
}

void AGwWeaponSpawner::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void AGwWeaponSpawner::Tick(float DeltaTime)
{
...
void AGwWeaponSpawner::OnConstruction(const FTransform& Transform)
{
...
void AGwWeaponSpawner::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void AGwWeaponSpawner::CheckForExistingOverlaps()
{
...
void AGwWeaponSpawner::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
	if (GetLocalRole() == ROLE_Authority && bIsWeaponAvailable && UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Pawn))
	{
		TSubclassOf<UGwInventoryItemDefinition> WeaponItemDefinition = WeaponDefinition ? WeaponDefinition->InventoryItemDefinition : nullptr;
...
void AGwWeaponSpawner::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &AGwWeaponSpawner::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void AGwWeaponSpawner::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &AGwWeaponSpawner::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void AGwWeaponSpawner::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void AGwWeaponSpawner::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, true);
}

void AGwWeaponSpawner::PlayPickupEffects_Implementation()
{
...
void AGwWeaponSpawner::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void AGwWeaponSpawner::OnRep_WeaponAvailability()
{
...
void AGwWeaponSpawner::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AGwWeaponSpawner, bIsWeaponAvailable);
}

int32 AGwWeaponSpawner::GetDefaultStatFromItemDef(const TSubclassOf<UGwInventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
	if (WeaponItemClass != nullptr)
	{
		if (UGwInventoryItemDefinition* WeaponItemCDO = WeaponItemClass->GetDefaultObject<UGwInventoryItemDefinition>())
		{
